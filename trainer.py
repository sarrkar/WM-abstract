...
def train_model(model, train_dataloader, val_dataloader, num_epochs=2000, learning_rate=0.001,):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels, task_index) in enumerate(train_dataloader):
            ...
            # Forward pass
            outputs = model(concatenated) # (batch_size, seq_len, output_size)
        ...
        epoch_val_loss = running_val_loss / total_batches
        epoch_val_accuracy = running_val_accuracy / total_batches
...
input_size = 10 + 18 # Since each sequence element is a scalar, number of ids and task index length
...
for name, (dataset_class, kwargs) in dataloaders.items():
    dataset = dataset_class(dataset_size=batch_size, std=0.05, **kwargs)

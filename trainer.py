...
def train_model(model, train_dataloader, val_dataloader, num_epochs=2000, learning_rate=0.001, verbose=False):
    ...
    for i, (inputs, labels, task_index) in enumerate(val_dataloader):
      ...
      for j in range(labels.shape[0]):
        ...
        if task_name_str == 'dms_identity':
          ...
          dms_position_confusion[identity1, identity2] += 1
    ...
    plt.figure(figsize=(8, 6))
    sns.heatmap(dms_position_confusion, annot=True, fmt='d')
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title(f'Confusion Matrix for Task dms_identity')
    plt.show()
...
for name, (dataset_class, kwargs) in dataloaders.items():
  dataset = dataset_class(dataset_size=batch_size, category_size=4, identity_size=4, std=0.15, pad_to=6, **kwargs)
